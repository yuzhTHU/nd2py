...
from ...utils.nn import GNN, PositionalEncoding
...
class NDformerModel(nn.Module):
    def __init__(self, config: NDformerConfig):
        ...
        self.embedder = nn.Embedding(self.config.n_words, self.config.d_emb)
        
        self.gnn_encoder = GNN(
            self.config.d_emb, 
            self.config.n_GNN_layers, 
            self.config.n_node_vars * self.config.d_data_feat,
            self.config.n_edge_vars * self.config.d_data_feat, 
            self.config.dropout
        )
        ...
        self.pe = PositionalEncoding(self.config.d_emb, self.config.dropout)
    ...
    def encode_graph(self, data_node, data_edge, edge_list, num_nodes, node_batch_idx):
        """
        ...
        gnn_out = self.gnn_encoder(data_node, edge_list, data_edge, num_nodes)

        # 2. 核心转换：稀疏变稠密
        # dense_nodes: (B_graph, MaxNodeNum, d_emb)
        # valid_mask: (B_graph, MaxNodeNum)，True 表示有效节点，False 表示 Pad 出来的假节点
        dense_nodes, valid_mask = to_dense_batch(gnn_out, node_batch_idx)
        ...
        # memory: (B_graph, MaxNodeNum, d_emb)
    ...
    def decode_sequence(self, partial_eq, memory, memory_key_padding_mask, seq2graph_idx=None, eq_pad_mask=None):
        """
        ...
        seq_emb = self.embedder(partial_eq)
    ...
    def forward(self, batch_dict):
        ...
        logits = self.decode_sequence(
            partial_eq=batch_dict["partial_eq"],
            memory=memory,
